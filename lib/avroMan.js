...
var createAvro = function () {
  ...
  Client.prototype = {
    ...
    validate: function (schema, datum) {
      ...
      switch (type) {
        ...
        case 'array':
          ...
          break
        default:
          throw new Error('R:Invalid schema type: ' + type)
      }
    ...
    readInt: function () {
      ...
      if (b > 0x7f) {
        throw new Error('Invalid int encoding.')
      }
    ...
    readLong: function readLong (bytes) {
    ...
    writeDatum: function (schema, datum) {
      ...
      if (type === 'object') {
        ...
        throw new Error('W:Undefined schema type ' + schema)
      } else {
        throw new Error('W:Unrecognized schema type: ' + type + schema)
      }
...
function readLong (bytes) {
  var b = bytes.shift() & 0xff
  var n = b & 0x7f
  if (b > 0x7f) {
    b = bytes.shift() & 0xff
    n ^= (b & 0x7f) << 7
    if (b > 0x7f) {
      ...
      if (b > 0x7f) {
        ...
        if (b > 0x7f) {
          // only the low 28 bits can be set, so this won't carry
          // the sign bit to the long
          return innerLongDecode(n, bytes)
        }
      }
    }
  }
...
function innerLongDecode (n, that) {
  var b = getByte(that)
  var l = Long.fromNumber(n, true)
  l = l.xor((b.and(0x7f).shiftLeft(28)))
  if (b.greaterThan(0x7f)) {
    b = getByte(that)
    l = l.xor((b.and(0x7f).shiftLeft(35)))
    if (b.greaterThan(0x7f)) {
      b = getByte(that)
      l = l.xor((b.and(0x7f).shiftLeft(42)))
      if (b.greaterThan(0x7f)) {
        b = getByte(that)
        l = l.xor((b.and(0x7f).shiftLeft(49)))
        if (b.greaterThan(0x7f)) {
          b = getByte(that)
          l = l.xor((b.and(0x7f).shiftLeft(56)))
          if (b.greaterThan(0x7f)) {
            b = getByte(that)
            l = l.xor((b.and(0x7f).shiftLeft(63)))
            if (b.greaterThan(0x7f)) {
              throw new Error('Invalid long encoding')
            }
          }
        }
      }
  ...
  return l.shiftRightUnsigned(1).xor(l.and(1).multiply(-1)).toNumber()

  function getByte (that) {
    return Long.fromNumber(that.readByte()).and(0xff)
  }
}
